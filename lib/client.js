...
// ===DATA-TIPS-END===
    ...
    // ------------------------------------------------------------------
    // TUNING DEFAULTS (overridable through dsh-loading-phrases.json,
    // served by the host half at /plugins/dsh-loading-phrases/config.json)
    // ------------------------------------------------------------------
    const DEFAULT_WITTY_MS = 5000;
    const DEFAULT_TIPS_MS = 10000;
    const DEFAULT_SHUFFLE = true;

    const CONFIG_URL = "/plugins/dsh-loading-phrases/config.json";

    // React is a platform seed word: the shell shares the singleton with
    // every client bundle through the module loader's require.
...
`;
    ...
    const COPY = {
      en: {
        ...
        wittyMs: "Witty dwell (ms)",
        tipsMs: "Tips dwell (ms)",
    ...
    async function apply(ctx) {
      ...
      try {
        if (typeof fetch === "function") {
          ...
          if (response.ok) {
            ...
            if (body !== null && typeof body === "object") section = body;
          }
        }
      } catch {
        section = {};
      ...
      // ------------------------------------------------------------------
      // SETTINGS PAGE (registered in the sidebar settings nav)
      // ------------------------------------------------------------------
      function Panel() {
        ...
        const [draft, setDraft] = React.useState(() => seed(currentSection));
        ...
        const build = () => ({
          mode: draft.mode,
          wittyIntervalMs: Number(draft.wittyIntervalMs) || DEFAULT_WITTY_MS,
          tipsIntervalMs: Number(draft.tipsIntervalMs) || DEFAULT_TIPS_MS,
          shuffle: Boolean(draft.shuffle),
          language: draft.language,
          phrases: { en: splitLines(draft.phrasesEn), zh: splitLines(draft.phrasesZh) },
          tips: { en: splitLines(draft.tipsEn), zh: splitLines(draft.tipsZh) },
        });
        ...
        const field = (label, control) =>
          React.createElement(
            "div",
            { className: "dshlp-field", key: label },
            React.createElement("span", { className: "dshlp-label" }, label),
            control,
          );
        ...
        return React.createElement(
          ...
          React.createElement("div", { className: "dshlp-panel-title" }, C.title),
          field(
            C.modeLabel,
            React.createElement(
              "select",
              { className: "dshlp-control", value: draft.mode, onChange: set("mode") },
              React.createElement("option", { value: "all" }, C.modeAll),
              React.createElement("option", { value: "witty" }, C.modeWitty),
              React.createElement("option", { value: "tips" }, C.modeTips),
              React.createElement("option", { value: "off" }, C.modeOff),
            ),
          ),
          field(
            C.wittyMs,
            React.createElement("input", {
              className: "dshlp-control",
              type: "number",
              min: "1000",
              value: String(draft.wittyIntervalMs),
              onChange: set("wittyIntervalMs"),
            }),
          ),
          field(
            C.tipsMs,
            React.createElement("input", {
              ...
              type: "number",
              min: "1000",
              value: String(draft.tipsIntervalMs),
              onChange: set("tipsIntervalMs"),
            }),
          ),
          field(
            C.shuffle,
          ...
          field(
            ...
            React.createElement(
              "select",
              { className: "dshlp-control", value: draft.language, onChange: set("language") },
              React.createElement("option", { value: "auto" }, C.languageAuto),
          ...
          React.createElement(
            ...
            React.createElement("span", { className: "dshlp-label" }, C.phrasesEn),
            React.createElement("textarea", {
              className: "dshlp-textarea",
              value: draft.phrasesEn,
              onChange: set("phrasesEn"),
            }),
          ),
          React.createElement(
            "div",
            { className: "dshlp-field-stack" },
            React.createElement("span", { className: "dshlp-label" }, C.phrasesZh),
            React.createElement("textarea", {
              className: "dshlp-textarea",
              value: draft.phrasesZh,
              onChange: set("phrasesZh"),
            }),
          ),
          React.createElement(
            "div",
            { className: "dshlp-field-stack" },
            React.createElement("span", { className: "dshlp-label" }, C.tipsEn),
            React.createElement("textarea", {
              className: "dshlp-textarea",
              value: draft.tipsEn,
              onChange: set("tipsEn"),
          ...
          React.createElement(
            "div",
            { className: "dshlp-field-stack" },
            React.createElement("span", { className: "dshlp-label" }, C.tipsZh),
            React.createElement("textarea", {
              className: "dshlp-textarea",
              value: draft.tipsZh,
              onChange: set("tipsZh"),
          ...
          React.createElement("div", { className: "dshlp-label" }, C.hint),
          React.createElement(
            "div",
            { className: "dshlp-actions" },
            React.createElement(
              "button",
              { type: "button", className: "dshlp-button dshlp-button-primary", onClick: save },
              C.save,
            ),
            React.createElement(
              "button",
              { type: "button", className: "dshlp-button", onClick: restore },
              C.restore,
            ),
            status === "saved"
              ...
              : null,
            status === "error"
              ? React.createElement("span", { className: "dshlp-status-error" }, C.saveFailed)
              : null,
          ),
        );
      }
      ...
      // Test seam: the settings panel re-applies config through the same
