...
// loader (`window.__ModuleLoader__`); no build step. The registration id
// must be the package name: the loader keys its graph rows by package name
// and rejects a bundle that registers under any other id. The PHRASES and
// TIPS blocks are generated from src/data/*.json by
// `node scripts/sync-data.js` — edit the JSON, then re-run the sync.

window.__ModuleLoader__.load({
  id: "@vincent-guo/dsh-loading-phrases",
  factory: (require) => {
    var module = { exports: {} };
...
`;
    ...
    async function apply(ctx) {
      ...
      // Minimal-write save (scheme C): a list that is empty or identical to
      // the built-in content is redundant in the user file — empty means
      // "fall back to built-ins" at runtime — so it collapses to []. Only a
      // genuinely customized list (a line removed, added, or reordered) is
      // written in full, because a non-empty list replaces the built-ins.
      const collapseList = (list, builtin) => {
        if (list.length === 0 || list.length !== builtin.length) return list;
        for (let i = 0; i < list.length; i++) {
          if (list[i] !== builtin[i]) return list;
        }
        return [];
      };
      ...
      const buildSection = (prefs, content) => ({
        ...
        phrases: {
          en: collapseList(splitLines(content.phrasesEn), PHRASES.en),
          zh: collapseList(splitLines(content.phrasesZh), PHRASES.zh),
        },
        tips: {
          en: collapseList(splitLines(content.tipsEn), TIPS.en),
          zh: collapseList(splitLines(content.tipsZh), TIPS.zh),
        },
      });
      ...
      exports.__test = { applySection, buildSection };
    }
