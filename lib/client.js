...
`;
    ...
    async function apply(ctx) {
      ...
      try {
        if (typeof fetch === "function") {
          const response = await fetch(
            "/plugins/dsh-loading-phrases/config.json?t=" + Date.now(),
          );
          if (response.ok) {
      ...
      const MODE = typeof section.mode === "string" ? section.mode : "all";
      // `off` yields entirely: no style, no observer — the original
      ...
      const TIPS_MS =
        typeof section.tipsIntervalMs === "number"
          ...
          : DEFAULT_TIPS_MS;
      const SHUFFLE = section.shuffle !== false;
      const LANGUAGE =
        typeof section.language === "string" ? section.language : "auto";
      const phraseOverride = section.phrases || { en: [], zh: [] };
      const tipsOverride = section.tips || { en: [], zh: [] };
