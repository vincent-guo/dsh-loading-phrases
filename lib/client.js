...
`;
    ...
    async function apply(ctx) {
        ...
        return React.createElement(
          ...
          field(
            ...
            React.createElement(
              "select",
              {
                ...
                value: prefs.mode,
                onChange: (event) => setPref("mode", event.target.value),
              },
          ...
          field(
            ...
            React.createElement("input", {
              ...
              min: "1",
              step: "1",
              value: String(prefs.wittySeconds),
              onChange: setPrefSeconds("wittySeconds"),
            }),
          ),
          field(
            C.tipsMs,
            React.createElement("input", {
              ...
              min: "1",
              step: "1",
              value: String(prefs.tipsSeconds),
              onChange: setPrefSeconds("tipsSeconds"),
            }),
          ),
          field(
            C.shuffle,
            React.createElement("input", {
              type: "checkbox",
              checked: Boolean(prefs.shuffle),
              onChange: (event) => setPref("shuffle", event.target.checked),
            }),
          ),
          field(
            C.language,
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              {
                type: "button",
                className: "dshlp-button dshlp-button-primary",
                onClick: saveContent,
              },
              C.save,
            ),
      ...
      // ------------------------------------------------------------------
      // OPEN-CONFIG ACTION (settings header, next to the shipped one)
      // ------------------------------------------------------------------
      function OpenConfigButton() {
        ...
        const available = meta !== null && meta.source === "user";
        const onClick = async () => {
          if (!available || meta.userPath === null) return;
          setBusy(true);
          setError(null);
          try {
            ...
            await connection.api.host.openPath({ path: meta.userPath });
          } catch (err) {
        ...
        const langLabel = locale !== undefined ? locale.getSnapshot().active : "en";
        const C2 = COPY[langLabel === "zh" ? "zh" : "en"];
        return React.createElement(
          "div",
          { className: "dshlp-actions" },
          React.createElement(
            ...
            {
              type: "button",
              className: "dshlp-button",
              disabled: !available || busy,
              title: available ? undefined : C2.openConfigHint,
              onClick,
            },
            C2.openConfig,
          ),
      ...
      if (slots !== undefined && slots.inject !== undefined) {
        ...
        slots.inject("settings.action", () =>
          slots.register(
            { name: "settings.action", id: "loading-phrases-config", order: 10 },
            () =>
              React !== undefined ? React.createElement(OpenConfigButton) : null,
          ),
        );
      }
