...
//
//   GET  /plugins/dsh-loading-phrases/config.json  read the effective config
//   POST /plugins/dsh-loading-phrases/config.json  save the config (writes
//        the package-root dsh-loading-phrases.json)
//
...
/** Read the raw config file; any failure yields `{}` so callers fall back. */
function readRaw() {
  try {
    ...
    handler: (req, res) => {
      ...
      const section =
        raw.loadingPhrases !== undefined && raw.loadingPhrases !== null
          ? raw.loadingPhrases
          : {}
      sendJson(res, 200, normalize(section))
    },
  })
  ctx.effect(() => disposer)
}
